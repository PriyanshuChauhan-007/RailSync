    ...
    <div className="time-distance-scroll" tabIndex={0} aria-label="Scrollable time-distance chart">
      <svg className={"time-distance-svg" + (bounded.zoom > 1 ? " is-zoomed" : "")} viewBox={"0 0 " + TD.width + " " + model.height} role="group" aria-label="Interactive time-distance chart"
        ...
        {ticks.map((tick, index) => (
          <g key={index}>
            <line x1={tick.x} y1={TD.top - 20} x2={tick.x} y2={model.height - 24} className="td-grid-time" />
            <text x={tick.x} y={20} textAnchor="middle" className="td-time-label td-time-label-top">{tick.label}</text>
            <text x={tick.x} y={model.height - 8} textAnchor="middle" className="td-time-label td-time-label-bottom">{tick.label}</text>
          </g>
        ))}
