      ...
      <div className="rn-candidate-controls">
        {["A", "B", "C"].map((id, i) => (
          <button
            type="button"
            key={id}
            aria-label={`Candidate ${id}: ${labels[i]}`}
        ...
        <button type="button" onClick={() => { setManual(null); setStep(0); setRun((value) => value + 1); }}>
          Replay evaluation
        </button>
      </div>
...
function StageLayer({ stage, corridor, example }) {
  ...
  if (stage === 0) {
    return <RailCallout title="TRAINS CLAIM CAPACITY" detail="Active train telemetry with real-time automatic block interlocking." anchor={center} />;
  }
  if (stage === 1) {
    const trd = corridor.tasks?.find((item) => item.department === "TRD");
    return (
      <g>
        {[...pair, trd].filter(Boolean).map((item, i) => {
          const at = sectionExtent(corridor, [item.section_id]);
          return (
            <g key={item.task_id}>
              <path d={`M${at.x + at.width / 2} 146V183L${110 + i * 175} 194`} className="rn-connector" />
              <MaintenanceMarker task={item} x={110 + i * 175} index={i} />
            </g>
          );
        })}
        <RailCallout title="MAINTENANCE NEEDS ACCESS" detail={`${task.task_id} · ${task.task_type} · ${task.duration_minutes} min`} anchor={center} />
      </g>
    );
  }
  if (stage === 2) {
    return (
      <g className="rn-conflict-story">
        ...
        <RailCallout title="WINDOWS COLLIDE · OCC INTERLOCKING" detail={`${train.train_id} · ${clockLabel(train.entry_time)}–${clockLabel(train.exit_time)} occupies ${task.section_id}`} anchor={center} tone="is-conflict" />
      </g>
    );
  }
  if (stage === 3) {
    return (
      <g>
        <rect x={extent.x} y="99" width={extent.width} height="57" rx="5" className="rn-working-section" />
        <RailCallout title="THIS SECTION IS UNDER EVALUATION" detail="Follow the candidate checks below. Other corridors stay in view." anchor={center} />
      </g>
    );
  }
  return (
    <g className="rn-coordination-story">
      {pair.map((item, i) => (
        <MaintenanceMarker task={item} x={center + (i ? 95 : -95)} y={200} index={i} merging key={item.task_id} />
      ))}
...
function CorridorBand({ corridor, focused, quiet, stage, paused }) {
  ...
  return (
    <section className={`rn-corridor ${focused ? "is-focused" : ""} ${quiet ? "is-quiet" : ""}`} aria-label={corridor.name}>
      <header className="rn-corridor-heading">
        <h3>{corridor.name}</h3>
        <span>
          {corridor.stations?.length ?? 0} stations · {corridor.sections?.length ?? 0} sections
          <i aria-hidden="true" />
        </span>
      </header>
      <div className="rn-band-scroll">
        <svg viewBox={quiet ? "0 65 1200 95" : "0 0 1200 240"} className="rn-band-scene" role="img" aria-label={`${corridor.name} railway schematic`}>
          ...
          <g className={`rn-traffic ${stage > 0 ? "is-background" : ""} ${focused && stage >= 2 ? "is-story-background" : ""}`}>
            {trains.map((train, index) => (
              <RailTrain key={train?.id ?? `train-${index}`} train={train} index={index} />
            ))}
          </g>
          {!quiet ? (
            <g key={stage} className={`rn-stage-layer ${paused ? "is-paused" : ""}`}>
              <StageLayer stage={stage} corridor={corridor} example={example} />
            </g>
          ) : null}
        </svg>
      </div>
...
export default function RailwaySchematic({ stage }) {
  ...
  const activeList = useMemo(() => {
    if (selected === "all") return demoCorridors;
    return demoCorridors.filter((c) => c.territory_id === selected);
  }, [selected]);
  ...
  return (
    <div className={`rn-network ${paused ? "is-paused" : ""}`} data-testid="rail-network">
      <div className="rn-network-toolbar">
        <div className="rn-corridor-selector" role="group" aria-label="Choose corridor">
          ...
          {demoCorridors.map((item, idx) => (
            <button
              type="button"
              key={item.territory_id ?? `corridor-${idx}`}
              aria-pressed={selected === item.territory_id}
              onClick={() => setSelected(item.territory_id)}
            >
      ...
      <div className="rn-canvas-caption">
        <span>4 authentic operational corridors · separate railway bands</span>
        <span>{selected === "all" ? "Stack Overview (4 Corridors Active)" : `Isolated: ${corridor?.name}`}</span>
      </div>
      <div className="rn-bands">
        {activeList.map((item, idx) => (
          <CorridorBand
            key={item.territory_id ?? `band-${idx}`}
            corridor={item}
            focused={focus === item.territory_id}
            quiet={selected === "all" ? false : false}
            stage={stage}
            paused={paused}
          />
        ))}
      ...
      {stage === 3 && corridor ? (
        <Evaluation key={corridor.territory_id ?? "eval"} example={corridorExample(corridor)} paused={paused} />
      ) : null}
    </div>
  );
}
